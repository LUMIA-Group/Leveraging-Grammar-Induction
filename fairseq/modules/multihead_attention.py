...
def masked_softmax(x, dim=-1, mask=None):
    max_x, _ = x.max(dim=dim, keepdim=True)
    e_x = torch.exp(x - max_x)
    if not (mask is None):
        e_x = e_x * mask
        out = e_x / (e_x.sum(dim=dim, keepdim=True) + 1e-8)
    return out
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if weight_mask is None:
            ...
            attn_weights_float = torch.sigmoid(attn_weights) * weight_mask
            # attn_weights_float = masked_softmax(attn_weights, dim=-1, mask=weight_mask)
